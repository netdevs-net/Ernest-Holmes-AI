  ...
  static searchQuestions(filters: QuestionFilters): QuestionHistory[] {
    ...
    if (filters.dateRange) {
      questions = questions.filter(q => {
        ...
        return questionDate >= filters.dateRange!.start && questionDate <= filters.dateRange!.end;
      });
    }
	...
	static updateQuestionSource(questionId: string, updates: { responsePreview?: string; source?: string }): void {
		if (typeof window === 'undefined') return;
		const questions = this.getQuestions();
		const question = questions.find(q => q.id === questionId);
		if (question) {
			if (updates.responsePreview) question.responsePreview = updates.responsePreview;
...
export function extractTags(question: string): string[] {
  ...
  if (lowerQuestion.includes('divine intelligence')) tags.push('divine-intelligence');
  ...
  if (lowerQuestion.includes('work') || lowerQuestion.includes('career')) tags.push('work');
  if (lowerQuestion.includes('health')) tags.push('health');
  if (lowerQuestion.includes('money') || lowerQuestion.includes('financial')) tags.push('money');
