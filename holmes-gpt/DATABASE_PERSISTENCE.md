...
```bash
# Docker run example
docker run -e DB_DIR=/custom/path -v /host/path:/custom/path ghcr.io/webdevs-net/holmes-gpt:latest
...
#### Basic Volume Mount
```bash
docker run -v holmes-data:/app/data ghcr.io/webdevs-net/holmes-gpt:latest
```
...
```bash
docker run -v /host/path/to/data:/app/data ghcr.io/webdevs-net/holmes-gpt:latest
```
